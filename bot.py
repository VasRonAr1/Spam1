...
import socks  # ### NEW ### нужно для socks.SOCKS5
...
########################################
BOT_TOKEN =  "7547201739:AAG34EIuv2kEk3z1xo_KWGoJesrbx1k5iSI"     # <-- Вставьте правильный токен бота
USER_STATE = {}        # user_id -> state
USER_TAGGER_TASKS = {} # user_id -> asyncio.Task

# ### NEW ### Прокси (пример из вашего короткого кода)
PROXY = (
    socks.SOCKS5,          # Тип прокси SOCKS5
    '45.149.133.209',      # IP
    63511,                 # Порт (SOCKS5)
    True,                  # SSL=True/False (True = пробуем TLS; если не работает, ставьте False)
    'eHma1Ljw',            # Логин
    'tVgB9Pvw'             # Пароль
)
...
########################################
# Создание/подключение Telethon-клиента (через прокси)
########################################
async def create_telethon_client(update: Update, context: ContextTypes.DEFAULT_TYPE, acc_number: int):
    ...
    # ### NEW ### Теперь передаём proxy=PROXY:
    if not acc_data['client']:
        client = TelegramClient(
            session_name,
            api_id,
            api_hash,
            proxy=PROXY  # <-- ВАЖНО: используем глобальную переменную PROXY
        )
